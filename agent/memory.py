...
# Basado en whatsapp-agent-kit de alanjmr21, adaptado para betting-bot
...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./data/agentkit.db")

# Ajustar URL si es PostgreSQL en producción
if DATABASE_URL.startswith("postgresql://"):
    DATABASE_URL = DATABASE_URL.replace("postgresql://", "postgresql+asyncpg://", 1)

# Asegurar que exista carpeta data
if "sqlite" in DATABASE_URL:
    # Extraer path del archivo db
    try:
...
async def obtener_historial(telefono: str, limite: int = 30) -> list[dict]:
    ...
    Args:
        ...
        limite: Máximo de mensajes a recuperar (default: 30)
...
async def obtener_conversaciones_activas(limite: int = 50) -> list[str]:
    """Obtiene lista de teléfonos con conversaciones recientes."""
    async with async_session() as session:
        query = (
